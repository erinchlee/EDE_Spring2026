...
```{r basics 1}
#1. use a sequence function : seq(from, to, by)
seq1 <- seq(100, 333, 3)
seq1

#2. use a mean function : mean(x)
mean_seq1 <- mean(seq1); mean_seq1

#3. use a sd function : sd(x)
sd_seq1 <- sd(seq1); sd_seq1

#4. use a basic math - / +
mean_minus <- mean(seq1)-sd(seq1); mean_minus
mean_plus <- mean(seq1)+sd(seq1); mean_plus
...
#6.#7.
#6.a Character vector
vector1 <- c("Robin", "Logan", "Erin", "Steven")
#6.b Numeric vector
vector2 <- c(85, 93, 78, 62)
#6.c Logical vector
...
> Answer: A data frame can contain columns of different data types (e.g., character, numeric, logical), whereas a matrix can contain only one data type.
...
function1 <- function(x){if(x>50) {"Pass"} else {"Fail"}}

#12. Create a function using ifelse()
function2 <- function(x){ifelse(x>50, "Pass", "Fail")}
...
vector_score <- c(33,55,72,86)
#function1(vector_score) #this is not working

#14b. Run the second function with the vector of test scores
function2(vector_score)
```
...
> Answer: The function that worked with a vector was "ifelse()". That’s because ifelse() is vectorized — it applies the condition to each element of a vector and returns a result for each one — whereas the ordinary if ... else statement in R only evaluates a single logical condition and can’t handle multiple elements at once.
